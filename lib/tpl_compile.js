			...
			if (src && src.toLowerCase().indexOf('http://') == -1) {
				...
				if (global.md5) {
					...
					if (node.attr('id') == "seajsnode") {
						node.replaceWith('<script src="' + global.hash_map + "(\'" + _k + "\')" + '" id="seajsnode"></script>');
					} else {
						node.replaceWith('<script src="' + global.hash_map + "(\'" + _k + "\')" + '"></script>');
					}
				} else {
					node.attr('src', src);
				}
			...
			if (pkg) {
				if (arr.length <= 1) {
					...
					if (global.md5) {
						...
						link = $('<link href="' + global.hash_map + "(\'" + _k + "\')" + '" rel="stylesheet" type ="text/css" />');
		...
		for (var key in global.ht_res) {
			...
			if (node.attr('href')) {
				if (global.md5) {
					_k = _k.replace(/[\\|/|\.]/ig, '_');
					global.static_map.push(_k + '=' + href);
					node.replaceWith($('<link href="' + global.hash_map + "(\'" + _k + "\')" + '" rel="stylesheet" type ="text/css" />'));
				} else {
					node.attr('href', href);
				}
			}
