...
function getConfig(options) {
	if (!fs.existsSync(path.join(global.static_directory, 'src/js', global.spm_directory, 'package.json'))) {
		throw new Error('can not find package.json in `' + global.spm_directory + '`!');
	}

	var pkg = grunt.file.readJSON(path.join(global.static_directory, 'src/js', global.spm_directory, 'package.json'));

	var deployConfig = pkg.spm;
	deployConfig.src = path.join(global.static_directory, 'src/js', global.spm_directory.replace(/[\\\/\.]/g, ''));
	deployConfig.family = pkg.family || global.spm_directory;
	deployConfig.outputDir = (options.outputDirectory || 'sea_modules').replace(/\\/g, '/').replace(/\/$/g, '');
	deployConfig.dist = path.join(global.static_directory, 'deploy/js', deployConfig.outputDir, deployConfig.family);
...
function md5Config(deployConfig) {
	var afterMd5 = function(fileChanges) {
		...
		fileChanges.forEach(function(obj) {
			...
			obj.newPath = obj.newPath.replace(deployConfig.dist + '/', '');
